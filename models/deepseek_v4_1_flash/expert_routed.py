...
def expert_routed(
    ...
    recv_mx_scale_mx = pl.tensor.view(
        recv_mx_scale,
        ...
        with pl.scope():
            for local_e in pl.parallel(N_LOCAL_EXPERTS):
                ...
                for tt in pl.parallel(e_tiles):
                    ...
                    with pl.spmd(D // ROUTE_TASK_TILE, name_hint="exp_route_output"):
                        wb_idx = pl.tile.get_block_idx()
                        ...
                        for dg in pl.range(ROUTE_TASK_TILE // ROUTE_D_OUT_TILE):
                            ...
                            y_fp32 = pl.row_expand_mul(y_fp32, route_weight_col)
...
def golden_expert_routed(tensors):
    ...
    w1_scale = decode_e8m0_codes(
        tensors["routed_w1_scale"], side="b",
    ).reshape(N_LOCAL_EXPERTS, K_SCALE, MOE_INTER)
    w3_fp8 = tensors["routed_w3"]
    w3_scale = decode_e8m0_codes(
        tensors["routed_w3_scale"], side="b",
    ).reshape(N_LOCAL_EXPERTS, K_SCALE, MOE_INTER)
    w2_fp8 = tensors["routed_w2"]
    w2_scale = decode_e8m0_codes(
        tensors["routed_w2_scale"], side="b",
    ).reshape(N_LOCAL_EXPERTS, H_SCALE, D)
    ...
    for e in range(N_LOCAL_EXPERTS):
        ...
        h = F.silu(gate) * up
        # V4.1 EP semantics apply route weight after the complete expert
        # output, when the result is combined back to the token owner.
...
def gen_routed_mx_weights(n_experts, dequant_std, seed_base=0):
    ...
    from models.deepseek_v4_1_flash.quantization import (
        pack_mx_b_scale as pack_b_scale,
        prepare_routed_weight_for_device,
        quantize_mxfp4_weight,
        unpack_mx_b_scale as unpack_b_scale,
    )
    ...
    for e in range(n_experts):
        ...
        w1_packed, w1_scale = quantize_mxfp4_weight(torch.randn(MOE_INTER, D))
        w3_packed, w3_scale = quantize_mxfp4_weight(torch.randn(MOE_INTER, D))
        w2_packed, w2_scale = quantize_mxfp4_weight(torch.randn(D, MOE_INTER))
...
def build_tensor_specs():
    ...
    from models.deepseek_v4_1_flash.quantization import host_quant_mxfp8_v41 as host_quant_mxfp8
    from models.deepseek_v4_1_flash.quantization import pack_mx_a_scale as pack_a_scale
    ...
    # view it as FP8.
    recv_x_pre = torch.zeros(N_LOCAL_EXPERTS, RECV_MAX, D, dtype=torch.uint8).view(torch.float8_e4m3fn)
